...
- **Batched Synthesis:** Each chapter is generated by a single worker process that loads the model once (`irodori_batch.py`), rather than launching the inference script per chunk. The model load was ~17 s of every ~20 s chunk before; batching cuts a 314-chunk chapter from roughly 108 minutes to about 33. A chunk that fails is reported and skipped, exactly as before.
...
| **Temp Folder** | Where intermediate working files (split sections/paragraphs/sentences, per-chunk `.wav` files, the chapter's `batch_jobs.json` plus the worker's `.log`, the run log) are written during a generation run, one subfolder per chapter, plus a shared `_silence` subfolder holding the three silence `.wav` files for the run. |
...
| **SilentCipher watermark** | **ON** by default, matching the TTS engine's own default: an inaudible marker identifying the audio as AI-generated is embedded in every chunk. **OFF** skips it, which also avoids the 48 kHz -> 44.1 kHz -> 48 kHz resample SilentCipher performs around the embed (its model is 44.1 kHz only) and saves roughly 60-200 ms per chunk. The difference is inaudible either way - with the watermark on, content above 22.05 kHz measures about 11 dB lower. |
