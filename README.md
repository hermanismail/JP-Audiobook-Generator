...
| **Save & Run** | Validates — including that the **Cover Art** image is still where the path says, offering to re-pick it if not, since a moved image would otherwise tag every chapter with no artwork — then writes `settings.json` and launches `run_audiobook.py` as a background process and opens the progress window (see below). Counts the chapters it actually intends to build — applying the **Regenerate existing chapters** rule — so the progress window doesn't promise "Chapter 1 of 18" for a run that means to build three. Says so plainly if every chapter is already generated. |
