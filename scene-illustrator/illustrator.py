...
illustrator.py
--------------
Core of the Scene Illustrator, v2 (2026-09-20). One image per chapter:

    read    the first 25% of each chapter -> a drawable moment, an image
            prompt, and a character roster that records where each character
            reappears (local LLM through llama-server)
    draw    3 samples for a chapter (FLUX.2 klein 9B, the shipped style
            sample attached, optionally one other chapter's final image)
    edit    change one thing in the working image and leave the rest alone
            (FLUX.1 Kontext dev), repeatedly, keeping the chain

The reasons behind every rule here are in DESIGN.md (v2 section); the ones
that cost real time to find are commented where they apply.
...
CLI (the window runs these as child processes, one code path):
    ...
    python illustrator.py draw  --book <name> --chapter chapter_003 [--count 3] [--anchor <png>]
    python illustrator.py edit  --book <name> --chapter chapter_003 --base <png>
                                --instruction-file <txt> [--count 3] [--extra <png>]
...
DEFAULT_SETTINGS = {
    ...
    "comfy_port": 8188,
    # draw: FLUX.2 klein 9B
    "draw_unet": "flux-2-klein-9b-Q4_K_M.gguf",
    "draw_clip": "qwen_3_8b_fp8mixed.safetensors",
    "draw_vae": "flux2-vae.safetensors",
    # edit: FLUX.1 Kontext dev
    "edit_unet": "flux1-kontext-dev-Q4_K_M.gguf",
    "edit_clip_l": "clip_l.safetensors",
    "edit_clip_t5": "t5xxl_fp8_e4m3fn_scaled.safetensors",
    "edit_vae": "ae.safetensors",
    "samples": 3,
    "edit_takes": 3,
    "last_book": "",
    "last_text_folder": "",
...
# The prompt rules that made the difference (DESIGN.md v2, M8): the style
# sample supplies the drawing style, and the background is kept out of the way.
...
def new_chapter():
    return {"prompt": "", "moment": "", "quote": "", "characters": [],
            "anchor": "",        # another chapter's final image, attached when drawing
...
def next_path(root, base, kind):
    """samples: sample_001.png; edits: edit_001.png - one flat folder per chapter."""
    folder = os.path.join(chapter_dir(root, base), kind)
    os.makedirs(folder, exist_ok=True)
    n = 1
...
def _engine(settings, mode, log):
    import comfy
    engine = comfy.ManagedComfy(settings, log)
    engine.check_models(mode)
    return engine


def run_draw(settings, book, base, count, anchor, log):
    """klein 9B: `count` samples for one chapter. The style sample is always
    attached; an anchor image (another chapter's final) may be too."""
    import random

    root = book_dir(settings, book)
    data = load_chapters(root)
    entry = data["chapters"].get(base) or new_chapter()
    prompt = entry["prompt"] or build_prompt(entry.get("moment", ""))
    if not prompt.strip():
    ...
    engine = _engine(settings, "draw", log)
    with engine:
        refs = [engine.put_reference(STYLE_IMAGE, f"si_{book}_style.png")]
        if anchor and os.path.isfile(anchor):
            refs.append(engine.put_reference(anchor, f"si_{book}_anchor.png"))
            log("SERVER drawing with the style sample and one anchor image")
        else:
            log("SERVER drawing with the style sample only")
        for i in range(count):
            out = next_path(root, base, "samples")
            log(f"TAKE {i + 1}/{count} start")
            secs = engine.run("draw", prompt, refs, random.randrange(2 ** 48), out,
                              on_progress=lambda v, m: log(f"PROGRESS {v} {m}"))
            log(f"SAMPLE {i + 1}/{count} {out} {secs}s")
    log(f"DONE drew {count}")


EDIT_TEMPLATE = ("Keep this drawing exactly as it is - same characters, same faces, same poses, "
                 "same clothes, same lines. Change ONLY this: {change}{keep}")


def edit_prompt(change, keep):
    """The phrasing that works. Measured (DESIGN.md v2, M6): an instruction
    must say what changes AND restate that everything else stays, or the change
    lands on the wrong person. The user writes the change; the tool writes the
    rest."""
    ...
    if not change.endswith("."):
        change += "."
    return EDIT_TEMPLATE.format(change=change, keep=keep_part) + " Everything else identical."


def run_edit(settings, book, base, count, base_image, instruction, extra, log):
    """Kontext dev: `count` takes of one change against `base_image`."""
    import random

    root = book_dir(settings, book)
    if not os.path.isfile(base_image):
        raise RuntimeError(f"image to edit not found: {base_image}")
    engine = _engine(settings, "edit", log)
    with engine:
        refs = [engine.put_reference(base_image, f"si_{book}_edit_base.png")]
        for i, path in enumerate(extra or []):
            if os.path.isfile(path):
                refs.append(engine.put_reference(path, f"si_{book}_edit_extra{i + 1}.png"))
        log(f"SERVER editing with {len(refs)} image(s)")
        for i in range(count):
            out = next_path(root, base, "edits")
            log(f"TAKE {i + 1}/{count} start")
            secs = engine.run("edit", instruction, refs, random.randrange(2 ** 48), out,
                              on_progress=lambda v, m: log(f"PROGRESS {v} {m}"))
            log(f"SAMPLE {i + 1}/{count} {out} {secs}s")
...
def main():
    ...
    e.add_argument("--count", type=int, default=3)
    ...
    try:
        ...
        if args.cmd == "draw":
            run_draw(settings, args.book, args.chapter, args.count, args.anchor, log)
            return
        ...
        run_edit(settings, args.book, args.chapter, args.count, args.base, instruction,
                 args.extra, log)
